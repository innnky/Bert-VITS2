...
class TextEncoder(nn.Module):
    def __init__(self,
        ...
        self.zh_bert_proj = nn.Conv1d(1024, hidden_channels, 1)
    ...
    def forward(self, x, x_lengths, tone, language, zh_bert, ja_bert, g=None):
        zh_bert_emb = self.zh_bert_proj(zh_bert).transpose(1,2)
        ja_bert_emb = self.ja_bert_proj(ja_bert).transpose(1,2)
        x = (self.emb(x)+ self.tone_emb(tone)+ self.language_emb(language)
             + zh_bert_emb +ja_bert_emb) * math.sqrt(self.hidden_channels)  # [b, t, h]
...
class SynthesizerTrn(nn.Module):
  ...
  """
    ...
    def forward(self, x, x_lengths, y, y_lengths, sid, tone, language, zh_bert, ja_bert):
        if self.n_speakers > 0:
            g = self.emb_g(sid).unsqueeze(-1)  # [b, h, 1]
        else:
            g = self.ref_enc(y.transpose(1,2)).unsqueeze(-1)
        x, m_p, logs_p, x_mask = self.enc_p(x, x_lengths, tone, language, zh_bert, ja_bert,g=g)
    ...
    def infer(self, x, x_lengths, sid, tone, language, zh_bert, ja_bert, noise_scale=.667, length_scale=1, noise_scale_w=0.8, max_len=None, sdp_ratio=0,y=None):
